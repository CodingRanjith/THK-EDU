...
import { Edit, Eye, FileSpreadsheet, Plus, Search, Trash2, ArrowUpRight, Clock, CheckCircle, AlertCircle } from 'lucide-react'
...
const EXPORT_COLUMNS = [
  ...
  { key: 'party_name', label: 'Party / Vendor' },
  ...
  { key: 'settled_date', label: 'Paid Date', getValue: (r) => formatDate(r.settled_date) },
...
export function AccountPayablePage() {
  ...
  const fetchData = useCallback(() => {
    ...
    Promise.all([financeApi.listPayables(query), financeApi.getPayableStats(query)])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (modal.mode === 'edit') {
        await financeApi.updatePayable(modal.item.id, payload)
        showSuccess('Updated', 'Payable entry updated.')
      } else {
        await financeApi.createPayable(payload)
        showSuccess('Created', 'Payable entry created.')
      }
      closeModal()
      fetchData()
    } catch (err) {
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await financeApi.deletePayable(item.id)
  ...
  const handleExport = () => {
    exportToExcel({
      filename: `payables-${formatPeriodLabel(period).replace(/\s+/g, '-')}.xlsx`,
      sheetName: 'Payables',
  ...
  const statCards = stats
    ? [
        { label: 'Total Payable', value: formatCurrency(stats.total_amount), icon: ArrowUpRight },
        { label: 'Paid', value: formatCurrency(stats.paid_amount), icon: CheckCircle },
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Track money owed by your organization — {formatPeriodLabel(period)}</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{total} entries · AP-001, AP-002, etc.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <form onSubmit={(e) => { e.preventDefault(); fetchData() }} className="flex flex-wrap gap-3">
          ...
          {loading ? (
            ...
            <p className="py-8 text-center text-sm text-muted-foreground">No payable entries for this period.</p>
          ) : (
              ...
              <table className="w-full min-w-[1000px] border-collapse text-sm">
                <thead>
                  <tr className="border-b bg-muted/40 text-left text-muted-foreground">
                    ...
                    <th className="px-3 py-3 font-medium">Party / Vendor</th>
      ...
      <FormModal open={modal.open} title={modal.mode === 'create' ? 'Add Payable' : modal.mode === 'edit' ? 'Edit Payable' : 'View Payable'} mode={modal.mode} onClose={closeModal} onSubmit={handleSubmit} submitting={submitting} size="xl">
        <div className="grid gap-4 sm:grid-cols-2">
          {modal.mode !== 'create' && (
            <FormField label="Entry Number"><FormInput value={modal.item?.entry_number || ''} disabled /></FormField>
          )}
          <FormField label="Party / Vendor Name" required>
          ...
          <FormField label="Category">
            <FormInput value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} disabled={isView} placeholder="e.g. Rent, Utilities" />
          </FormField>
